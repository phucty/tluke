...
def _create_luke_config(bert_config, entity_vocab_size, entity_emb_size):
    return LukeConfig(entity_vocab_size=entity_vocab_size,
...
def test_entity_embedding(bert_config):
    ...
    for n, (entity_id, position_id_list, token_type_id) in enumerate(
            zip(entity_ids, position_ids, token_type_ids)):
        entity_emb = entity_embeddings.entity_embeddings.weight[entity_id]
        token_type_emb = entity_embeddings.token_type_embeddings.weight[
            token_type_id]
        position_emb_list = [
            entity_embeddings.position_embeddings.weight[p]
            for p in position_id_list if p != -1
        ]
        if position_emb_list:
            position_emb = functools.reduce(
                operator.add, position_emb_list) / len(position_emb_list)
            target_emb = entity_embeddings.LayerNorm(
                (entity_emb + position_emb + token_type_emb))
        else:
            target_emb = entity_embeddings.LayerNorm(
                (entity_emb + token_type_emb))
