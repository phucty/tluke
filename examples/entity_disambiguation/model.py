...
class TLukeEntityEmbeddingsForEntityDisambiguation(TableEntityEmbeddings):
    ...
    def forward(
        ...
        position_row_ids: torch.LongTensor = None,
        position_col_ids: torch.LongTensor = None,
    ):
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(entity_ids)

        if position_row_ids is None:
            position_row_ids = torch.zeros_like(entity_ids)

        if position_col_ids is None:
            position_col_ids = torch.zeros_like(entity_ids)
...
class TLukeForEntityDisambiguation(TLukePreTrainedModel):
    def __init__(self, config: LukeTableConfig):
        super().__init__(config)
        self.tluke = LukeTableModel(config)
        self.entity_predictions = EntityPredictionHead(config)
        self.tluke.embeddings = BertTableEmbeddings(config)
        self.tluke.entity_embeddings = TLukeEntityEmbeddingsForEntityDisambiguation(config)

        self.post_init()

    def tie_weights(self):
        super().tie_weights()
        self._tie_or_clone_weights(self.entity_predictions.decoder, self.tluke.entity_embeddings.entity_embeddings)
    ...
    def forward(
        ...
        outputs = self.tluke(
            ...
            word_position_ids=position_ids,
            entity_ids=entity_ids,
            entity_attention_mask=entity_attention_mask,
            entity_segment_ids=entity_token_type_ids,
            entity_position_ids=entity_position_ids,
            # head_mask=head_mask,
            # inputs_embeds=inputs_embeds,
            # output_attentions=output_attentions,
        ...
        word_last_hidden_state, entity_last_hidden_state, pooler_output = outputs
        logits = self.entity_predictions(entity_last_hidden_state).view(-1, self.config.entity_vocab_size)
        ...
        return EntityDisambiguationOutput(
            ...
            entity_hidden_states=None,  # outputs.entity_hidden_states,
